...
Rendering Layer: 使用系統原生 WebView (WebKit on iOS, WebView on Android) 執行 Solid.js UI。
Native Bridge: 透過 Tauri 的 invoke 模式，將耗能運算（加密、影像處理）交由 Rust 處理。
Primitive API: 封裝了 Solid 專屬的 createCamera()、createStorage() 等 Signals，讓原生功能像狀態一樣易用。
